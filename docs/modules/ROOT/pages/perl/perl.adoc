...
either xref:#_perl_numbers[numbers], xref:#_perl_strings[strings], or more
complex such as a xref:#_perl_list[list]. Data is held as value.
...
xref:#_perl_variables[Perl variables].
...
xref:#_perl_numbers[number], complex expression with
xref:#_perl_operators[operators], or a
xref:#_perl_subroutine[function (aka subroutine)], call.
...
xref:#_perl_modules[modules], and other objects. Check it
...
xref:#_perl_numbers[numbers] and xref:#_perl_strings[strings],
...
xref:#_perl_array[array], not xref:#_perl_hash[hashes]. In addition, the
interpolation is only applied to the double-quoted string, but not the
...
xref:#_perl_if_Statement[if], xref:#_perl_while_Loop[while],
xref:#_perl_given[given], etc., to control the flow of the program. The
following are logical operators in Perl:
...
A Perl list is a sequence of xref:#_perl_variables[scalar]
...
A
xref:#_perl_variables[scalar variable]
...
Perl sort function xref:#_perl_sort[Perl sort function].
